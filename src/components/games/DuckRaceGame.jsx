...
export function DuckRaceGame({ questions, teams, onAddPoints }) {
  const [currentQIndex, setCurrentQIndex] = useState(0);
  const [duckPositions, setDuckPositions] = useState(teams.map(() => 0));
  ...
  const handleAnswer = (optLabel) => {
    ...
    if (isCorrect) {
      ...
      // Advance active team's duck by 25%
      const newPositions = [...duckPositions];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', padding: '10px 20px', width: '100%', maxWidth: '950px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-bright)' }}>
            🦆 Đua Vịt Tri Thức - Đua Vịt Trên Sông
          </h2>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
            Trả lời đố vui đúng để chú vịt của đội bơi nhanh về đích!
          </p>
        </div>

        <button className="btn btn-secondary btn-sm" onClick={handleResetRace}>
          <RotateCcw size={16} /> Đặt Lại Cuộc Đua
        </button>
      </div>
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.8rem', color: '#e0f2fe', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '8px' }}>
          <span>🌊 BỜ SÔNG XUẤT PHÁT</span>
          <span>🏁 PHAO VỀ ĐÍCH</span>
        </div>

        {teams.map((team, idx) => {
          const pos = duckPositions[idx] || 0;
          ...
          return (
            <div 
              ...
              style={{
                ...
                padding: '8px 12px',
                borderRadius: '16px',
                background: activeTeamIndex === idx ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.2)',
                border: activeTeamIndex === idx ? `2px solid ${team.color}` : '1px solid rgba(255,255,255,0.1)'
              }}
            >
              <div style={{ width: '130px', fontWeight: 800, color: '#fff', fontSize: '0.85rem', flexShrink: 0 }}>
                {team.name}
              </div>
              ...
              <div style={{
                ...
                <div style={{ position: 'absolute', right: '10px', fontSize: '1.2rem' }}>
                  🚩
                </div>
      ...
      {winnerTeam ? (
        <div className="glass-panel" style={{ width: '100%', padding: '36px', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '2rem', fontWeight: 900, color: '#fbbf24' }}>
            🎉 CHIẾN THẮNG THUỘC VỀ chú VỊT CỦA {winnerTeam.toUpperCase()}!
          </h2>
          <button className="btn btn-primary btn-lg" onClick={handleResetRace} style={{ marginTop: '16px' }}>
            Đua Vịt Ván Mới
          </button>
        </div>
      ) : (
        <div className="glass-panel" style={{ width: '100%', padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              Đến lượt vịt của: <strong style={{ color: teams[activeTeamIndex]?.color || '#fff' }}>{teams[activeTeamIndex]?.name}</strong>
            </span>
          </div>
          ...
          {answerState && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderRadius: '12px', background: answerState === 'correct' ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)' }}>
              <div style={{ fontWeight: 800, color: answerState === 'correct' ? '#6ee7b7' : '#fca5a5' }}>
                {answerState === 'correct' ? `🎉 ${teams[activeTeamIndex]?.name} TRẢ LỜI ĐÚNG! VỊT BƠI TIẾN THÊM +25%!` : `❌ CHƯA CHÍNH XÁC!`}
              </div>
