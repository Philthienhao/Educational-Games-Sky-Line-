...
export function LessonPlannerHub({ currentUser }) {
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      setMindmapData(mindmapRes);
      setWorksheetData(worksheetRes);
    } catch (err) {
      ...
      <div style={{
        ...
        <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'center' }}>
          ...
          >
            ...
            <span>{loading ? '⏳ AI đang tự động biên soạn Slide, Mindmap & Phiếu A4...' : '🪄 Khởi Tạo Giáo Án AI Multi-Format'}</span>
          </button>
        </div>
